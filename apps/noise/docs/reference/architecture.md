...
| --- | --- | --- |
| Language | Go 1.25+ | Single static binary per platform. |
...
| Persistence | SQLite (modernc.org/sqlite), local Markdown + YAML | Domain data is stored in Markdown with YAML frontmatter; SQLite tracks metadata, versions, knowledge base, sync data, and project stats; schema defined in [`internal/infra/db/schema.go`](../internal/infra/db/schema.go). |
| Voice | whisper.cpp (Go bindings) + malgo | Local speech-to-text with push-to-talk. Models auto-download on first use. Optional CGO for native performance. |
| Sync | net/http + gorilla/websocket | Embedded HTTP/WebSocket server for PWA companion. Local network sync with device pairing. |
...
## 3. System Layers
1. **Presentation** – Bubble Tea models in `internal/ui/` manage screen routing, layout composition, and styling via Lip Gloss. Includes voice indicator, sync status, and idea inbox components.
2. **Application** – Services in `internal/app/` orchestrate editor behaviors, AI agent workflows, export logic, tempo/theme facilities, and voice-to-text coordination (`voice_service.go`).
3. **Domain** – Structs in `internal/domain/` define Songs, Sections, Lines, QualityScore, and related entities.
4. **Infrastructure** – Packages under `internal/infra/` provide:
   - `db/` – SQLite persistence with repository patterns
   - `voice/` – Audio capture (malgo), whisper.cpp integration, model management
   - `sync/` – HTTP/WebSocket server, device pairing, media storage for PWA companion
   - `ollama/` – AI client adapters
...
## 5. UI Architecture
Key UI components (referenced in [design-system.md](./design-system.md)):
...
- Architecture, prompts, and response parsing are described in [ai-integration.md](./ai-integration.md).
...
## 7. Theme & Styling
[design-system.md](./design-system.md) defines the core palette, Lip Gloss styles, animation patterns, and component guidance:
...
## 9. Export & File I/O
Export services turn captured ideas into shareable artifacts per [export_formats.md](./export_formats.md):
...
- **Local-first privacy** – No telemetry or cloud storage. Voice processing and sync happen entirely on-device/local network.
- **Dependency isolation** – Single binary distribution. CGO is optional (only needed for native whisper.cpp performance). Pure Go fallback available.
- **Auto-download safety** – Whisper models download from Hugging Face with checksum verification; sync media stored in isolated directories.
...
- **Fallback behavior** – AI timeouts fall back to helpful messaging; voice service gracefully degrades if models unavailable; theme manager defaults to Amber Night on invalid config; export commands validate targets before writing.
