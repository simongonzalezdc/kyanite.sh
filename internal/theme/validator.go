...
func ValidateTheme(t Theme) []string {
	...
	if cr := ContrastRatio(tx,bg); cr < 4.5 { warnings = append(warnings, fmt.Sprintf("Text contrast too low: %.2f:1", cr)) }
	if cr := ContrastRatio(pr,bg); cr < 3.0 { warnings = append(warnings, fmt.Sprintf("UI contrast too low: %.2f:1", cr)) }
	return warnings
}

func parseHex(hex string) color.Color {
	h := strings.TrimPrefix(hex, "#")
	if len(h) != 6 { return color.White }
